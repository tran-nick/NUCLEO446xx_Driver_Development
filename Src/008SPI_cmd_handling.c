...
int main(void)
{
	...
	*/
	...
	while(1)
	{
		...
		if( SPI_VerifyResponse(ackbyte))	//if slave supports command, it will send response ACK
		{
			args[0] = LED_PIN;
			args[1] = LED_ON;
		...
		if( SPI_VerifyResponse(ackbyte))
		{
			...
			//another dummy read to clear RXNE, because when you send data, you also receive because of shift registers
			//so receive to clear data from send data of args
			SPI_ReceiveData(SPI2,&dummy_read,1);
