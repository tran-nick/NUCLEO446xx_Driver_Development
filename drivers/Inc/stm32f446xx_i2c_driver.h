...
typedef struct
{
	...
	uint8_t 		*pTxBuffer; /* !< To store the app. Tx buffer address for interrupt> */
	uint8_t 		*pRxBuffer; /* !< To store the app. Rx buffer address for interrupt> */
	uint32_t 		TxLen;		/* !< To store Tx len for interrupt > */
	uint32_t 		RxLen;		/* !< To store Rx len for interrupt > */
	uint8_t 		TxRxState;	/* !< To store Communication state for interrupt -- only one parameter because I2C half duplex> */
	uint8_t 		DevAddr;	/* !< To store slave/device address for interrupt> */
	uint32_t 		RxSize;		/* !< To store Rx size  for interrupt> */
	uint8_t 		Sr;			/* !< To store repeated start for interrupt > */
}I2C_Handle_t;
...
void I2C_SlaveManageCallBackEvents(I2C_RegDef_t *pI2Cx , uint8_t EnorDi);	//enable/disable interrupt control bits
